...
def letter_grades(highest):
    ...
    offset = round((highest - 40) / 4)

    grades = [
        grades[0] + offset * index
...
def student_ranking(student_scores, student_names):
    ...
    combined = sorted(zip(student_scores, student_names), reverse=True)
    ...
    for index, score in enumerate(scores):
        ranks.append(f'{index+1}. {names[index]}: {score}')
...
def perfect_score(student_info):
    ...
    student_info.sort(key=lambda item: item[1], reverse=True)
