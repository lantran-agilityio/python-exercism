...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    band_mapper = {'green': 80, 'orange': 60, 'red': 30, 'black': 0}

    for key, value in band_mapper.items():
        if efficiency >= value:
            return key
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    low_threshold = threshold * 90 / 100
    normal_threshold = (threshold - threshold * 10 / 100,threshold+threshold * 10 / 100)

    if normal_threshold[0] <= value <= normal_threshold[1]:
        return 'NORMAL'
